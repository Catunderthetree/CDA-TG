...
def SolMOSA(config):
    ...
    accounts, contract_json, contract_name, deployed_bytecode, bytecode, abi \
        = get_ETH_properties(ETH_port, max_accounts, accounts_file_location, contract_json_location)
    ...
    for i in range(search_budget):
        ...
        logging.info("Run time: {}".format(rtime))
                ...
                logging.info("{} out of {} branches have been covered"
                             .format(len([test for test, relevant in
                                     ...
                                     len([test for test, relevant in
                                          zip(archive, relevant_targets) if
                                          relevant])))
                rrtime = datetime.datetime.now() - start_time
                logging.info("Run time: {}".format(rrtime))
        archive[i] = best_test
    return archive
